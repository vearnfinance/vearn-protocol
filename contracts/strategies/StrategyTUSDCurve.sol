...
contract StrategyTUSDCurve {
    ...
    address constant public v = address(0x73a052500105205d34Daf004eAb301916DA8190f);
    ...
    function withdrawTUSD(uint256 _amount) internal returns (uint) {
        ...
        if (_vdai > 0) {
            ...
            ICurveFi(curve).exchange(0, 3, _vdai, 0);
        }
        if (_vusdc > 0) {
            IERC20(vusdc).safeApprove(curve, 0);
            IERC20(vusdc).safeApprove(curve, _vusdc);
            ICurveFi(curve).exchange(1, 3, _vusdc, 0);
        }
        if (_vusdt > 0) {
            IERC20(vusdt).safeApprove(curve, 0);
            IERC20(vusdt).safeApprove(curve, _vusdt);
            ICurveFi(curve).exchange(2, 3, _vusdt, 0);
        }
        
        uint _before = IERC20(want).balanceOf(address(this));
        vERC20(vtusd).withdraw(IERC20(vtusd).balanceOf(address(this)));
    ...
    function _withdrawAll() internal {
        ...
        if (_vvcrv > 0) {
            ...
            withdrawTUSD(IERC20(vcrv).balanceOf(address(this)));
        }
    }
